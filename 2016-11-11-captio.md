...
·      
The user don’t need to use the
lateral menu if you only want to take the photo, the button was moved to the
main screen. 

·      
Added an automatic trimming of the
photo (improving image- capture process)

·      
Added the option of select a
picture from gallery

Migrate some functionality from Azure **IaaS****to****PaaS** (migrating some heavy process to a Service that can auto
scale).
